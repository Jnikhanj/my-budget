(function () {
  ...
  style.textContent = `
    ...
    @media (max-width: 380px) {
      .bar-chart { height: 210px !important; }
  ...
  window.renderBudgetPaceChart = function renderBudgetPaceChart() {
    ...
    const activePoint = actualPoints[currentIndex] || actualPoints[actualPoints.length - 1];
    ...
    chart.innerHTML = `
      <div class="pace-chart" role="img" aria-label="Budget pace chart. Actual spend ${money(status.spent)} compared with expected pace ${money(expectedNow)}.">
        <svg class="pace-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-hidden="true">
          ...
          <text class="pace-end-label" x="${width - padX}" y="${Math.max(12, activePoint.y - 10)}">${money(activePoint.value)}</text>
        </svg>
        ...
        <div class="pace-footer">
          ...
          <div class="pace-status">${escapeValue(statusText)}</div>
        </div>
      </div>
    `;
  };

  renderWeeklyChart = window.renderBudgetPaceChart;

  const previousRenderAnalytics = renderAnalytics;
  renderAnalytics = function renderAnalyticsWithBudgetPace() {
    previousRenderAnalytics();
    window.renderBudgetPaceChart();
  };
