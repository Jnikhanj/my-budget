...
// PERSISTENT FORM STATE - This won't get cleared
let formData = {
  amount: "",
  merchant: "",
  categoryId: "",
...
function saveManualTransaction() {
  ...
  try {
    ...
    const category = categoryById(categoryId) || categoryByName("Other") || state.categories[0];
    const expense = {
...
function wireEvents() {
  ...
  // Amount input - sync to formData immediately
  $("amountInput").addEventListener("input", (e) => {
    formData.amount = e.target.value;
  });

  // Merchant input - sync to formData and show suggestions
  $("merchantInput").addEventListener("input", (e) => {
    formData.merchant = e.target.value;
  ...
  $("merchantInput").addEventListener("blur", () => {
    setTimeout(() => $("merchantSuggestions").classList.remove("show"), 180);
  });

  // Category select - sync to formData
  $("categoryInput").addEventListener("change", (e) => {
    formData.categoryId = e.target.value;
  });

  // Date input - sync to formData
  $("dateInput").addEventListener("change", (e) => {
    formData.date = e.target.value;
  });

  // Note input - sync to formData
